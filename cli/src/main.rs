//! cargo-vouch — prove AI-generated Rust is panic-free (don't just test it).
//!
...
//!
//!   cargo-vouch <file.rs | dir>...      verify a file, many files, or a whole dir
//!   cargo-vouch --prove '<expr>' <file> prove a postcondition over `result`/inputs
//!   cargo-vouch --emit <file.rs>        print the generated harness, don't run
//!   cargo-vouch --json <path>...        machine-readable {results, summary}
//!   cargo-vouch --selftest              check the Kani wiring is trustworthy
//!   cargo-vouch --bound N / --unwind N  tune the BMC depth
//!   cargo-vouch --fail-on <level>       gate strictness: bug|unguarded|inconclusive
...
/// Loops are cargo-vouch's out-of-niche signal; used only to tailor the INCONCLUSIVE
...
fn helper() -> String {
    format!(
        r#"// auto-generated by cargo-vouch
...
fn harness_for(
    ...
    let tail = match postcond {
        Some(expr) => format!(
            "    let result = {name}({call});\n    kani::assert({expr}, \"vouch postcondition\");\n    let _ = &result;"
        ),
...
mod tests {
    ...
    fn postcondition_binds_result_and_asserts() {
        ...
        assert!(lib.contains(r#"kani::assert(result >= a, "vouch postcondition");"#));
    }
    ...
    fn collect_rs_finds_rs_recursively_sorted() {
        ...
        use std::fs;
        let root = std::env::temp_dir().join("vouch-collect-test");
...
fn verify_file(src: &str, slot: usize) -> Vec<(String, Verdict)> {
    ...
    let base = std::env::temp_dir().join(format!("cargo-vouch-{}-multi-{slot}", std::process::id()));
...
fn batch(files: &[String], json: bool) -> i32 {
    ...
    if !json {
        println!(
            "{DIM}cargo-vouch batch — {} file(s), {workers} workers, ≤{TIMEOUT_SECS}s/mode each{X}\n",
            files.len()
        );
    }
...
fn main() {
    ...
    if args.first().map(|s| s.as_str()) == Some("vouch") {
        args.remove(0); // invoked as `cargo vouch ...`
    }
    if args.iter().any(|a| a == "--version" || a == "-V") {
        println!("cargo-vouch {}", env!("CARGO_PKG_VERSION"));
        return;
    }
    ...
    if args.is_empty() || args.iter().any(|a| a == "--help" || a == "-h") {
        print!(
            "\
cargo-vouch {ver} — prove AI-generated Rust is panic-free (don't just test it)

USAGE:
    cargo-vouch <file.rs>...             verify one file, or many (batch table)
    cargo-vouch --prove '<expr>' <file>  prove a postcondition over `result`/inputs
    cargo-vouch --emit <file.rs>         print the generated Kani harness, don't run
    cargo-vouch --json <file.rs>...      machine-readable results (single or batch)
    cargo-vouch --selftest               check Kani is wired correctly (trust guard)
...
",
    ...
    if let Some(i) = args.iter().position(|a| a == "--prove") {
        match args.get(i + 1).cloned() {
            ...
            None => {
                eprintln!("usage: cargo-vouch --prove '<bool expr over result/inputs>' <file.rs>");
                std::process::exit(2);
            }
        }
    }
    ...
    let raw: Vec<String> = args
        ...
        .collect();
    if raw.is_empty() {
        eprintln!("usage: cargo-vouch [--emit] [--prove '<expr>'] <file.rs | dir>...");
        std::process::exit(2);
    }
    // A directory argument expands to every .rs file under it (recursive) — so
    // `cargo-vouch src/` verifies a whole crate without a shell glob.
    ...
    if files.is_empty() {
        eprintln!("cargo-vouch: no .rs files found in {}", raw.join(", "));
        std::process::exit(2);
    }
    ...
    if let Some(expr) = prove {
        ...
        let dir =
            std::env::temp_dir().join(format!("cargo-vouch-{}-{name}-prove", std::process::id()));

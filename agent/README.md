...
A dev agent that closes the loop around cargo-aiv:

```
spec ─▶ Claude writes a Rust fn ─▶ cargo-aiv (Kani BMC) ─▶ 🔴/🟡/⏱/⏭ → feed the
                                                            verdict + witness back
                                                          ─▶ ✅ VERIFIED → done
```

The point: an LLM confidently calls its own code "correct" — cargo-aiv's whole
...
# cargo-aiv must be built (../cli) or on PATH, and Kani installed:
...
```bash
.venv/bin/python aiv_agent.py "a function that returns the average of a slice of i32"
```
...
| `--bound/--unwind/--str-bound/--str-unwind N` | passed through to cargo-aiv |
| `--out PATH` | write the final proven function to a file |
| `--cargo-aiv PATH` | explicit cargo-aiv binary |

Exit code `0` = proven, `1` = gave up unproven, `2` = setup/API error.

## Scope

The agent inherits cargo-aiv's niche: it's told to write **loop-light** functions
over supported param types. It shines on the class of code cargo-aiv verifies —
